...
@Component({
  ...
  <section class="page-card">
    ...
    <table mat-table [dataSource]="departments">
      ...
      <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef>Actions</th><td mat-cell *matCellDef="let d"><button mat-button (click)="edit(d)">Edit</button><button mat-button color="warn" (click)="remove(d.id)">Suppr</button></td></ng-container>
...
export class DepartmentsComponent implements OnInit {
  ...
  load(): void { this.api.getDepartments().subscribe(d => this.departments = d); }
  edit(item: Department): void { this.editId = item.id; this.form.patchValue(item); }
  remove(id: number): void { this.api.deleteDepartment(id).subscribe(() => this.load()); }

  save(): void {
    if (this.form.invalid) return;
    const payload = this.form.getRawValue();
    const request$ = this.editId ? this.api.updateDepartment(this.editId, payload) : this.api.createDepartment(payload);
    request$.subscribe(() => { this.load(); this.form.reset({ name: '', code: '' }); this.editId = undefined; });
  }
}
